...
namespace Lucy {
	...
	void ViewportRenderer::Dispatch(Scene& scene) {
		auto& uniformBuffer = s_GeometryPipeline->GetUniformBuffers<VulkanUniformBuffer>("Camera");

		EditorCamera& camera = scene.GetEditorCamera();
		auto vp = camera.GetVP();
		uniformBuffer->SetData((void*)&vp, sizeof(vp), 0);
	...
	void ViewportRenderer::GeometryPass() {
		Renderer::RecordStaticMeshToCommandQueue(s_GeometryPipeline, [](Ref<MeshDrawCommand> staticMeshDrawCommand) {
			...
			for (uint32_t i = 0; i < submeshes.size(); i++) {
				...
				pushConstantData.MaterialIndex = submesh.MaterialIndex;
				
				meshPushConstant.SetData(&pushConstantData, sizeof(pushConstantData));
